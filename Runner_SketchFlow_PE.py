...
from model.utils import DataLoader, DataLoaderSketchFlow, DataLoaderSketchFlowPosition
from model.final_future_prediction_with_memory_spatial_sumonly_weight_ranking_top1 import convAE as ConvAEPred
from model.final_future_prediction_with_memory_spatial_sumonly_weight_ranking_top1 import GCNNet, GraphSageNet, \
    GatedGCNNet, MyGCNNet, ConvAESketchFlow
    ...
    def __init__(self, args):
        ...
        if self.args.has_sketch_flow:
            if self.args.feature_size > 0:
                self.train_dataset = DataLoaderSketchFlowPosition(
                    self.train_folder, self.args.sketch_flow_train_folder,
                ...
                self.test_dataset = DataLoaderSketchFlowPosition(
                    self.test_folder, self.args.sketch_flow_test_folder,
                    transforms.Compose([transforms.ToTensor()]), resize_height=self.args.h,
                    resize_width=self.args.w, time_step=self.args.t_length - 1,
                    feature_size=self.args.feature_size)
            else:
                self.train_dataset = DataLoaderSketchFlow(
                    self.train_folder, self.args.sketch_flow_train_folder,
                    transforms.Compose([transforms.ToTensor()]), resize_height=self.args.h,
                    resize_width=self.args.w, time_step=self.args.t_length-1)
                self.test_dataset = DataLoaderSketchFlow(
                    self.test_folder, self.args.sketch_flow_test_folder,
    ...
    def train(self):
        ...
        for epoch in range(self.args.epochs):
            ...
            for j, now_data in enumerate(self.train_batch):
                ...
                if self.args.has_sketch_flow:
                    if self.args.feature_size > 0:
                        imgs, batched_graph, nodes_num_norm_sqrt, edges_num_norm_sqrt, positions = now_data
                    else:
                ...
                if self.args.method == 'pred':
                    if self.args.has_sketch_flow:
                        ...
                        if self.args.feature_size > 0:
                            positions = positions.to(self.args.device)
                        (outputs, _, _, m_items, _, _, separateness_loss, compactness_loss) = self.model.forward(
                            imgs[:, 0:12], m_items, batched_graph, nodes_feat,
                            edges_feat, nodes_num_norm_sqrt, edges_num_norm_sqrt, True, positions)
                    else:
            ...
            if self.args.method == 'pred':
                if self.args.has_sketch_flow:
                    ...
                    (outputs, feas, _, m_items_test, _, _, _, _, _, compactness_loss) = self.model.forward(
                        imgs[:, 0:3 * 4], m_items_test, batched_graph, nodes_feat,
                        edges_feat, nodes_num_norm_sqrt, edges_num_norm_sqrt, False, positions)
                else:
...
def get_arg(gpu_id=0, run_name="demo", has_sketch_flow=True,
            ...
            hidden_dims=None, which_sketch="sketch_10_40_25", feature_size=0):
...
if __name__ == '__main__':
    ...
    feature_size = 32  # [0, 32]，其中0表示不按位置扩展

    seed_setup(seed)
    runner = Runner(args=get_arg(
        gpu_id=gpu_id, has_sketch_flow=has_sketch_flow, which_gnn=which_gnn, feature_size=feature_size,
        hidden_dims=hidden_dims, which_sketch=which_sketch, which_sketch_flow=which_sketch_flow,
        run_name="{}_{}seed_{}_{}layer_{}_position{}".format(
            which_sketch, seed, which_gnn.__name__,
            len(hidden_dims), which_sketch_flow.replace("/", "_"), feature_size)))
