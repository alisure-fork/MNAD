...
class SketchFlowGraph(object):
    ...
    def __init__(self, sketch_flow_txt, image_size):
        ...
        self.target = sum([one.displacement for one in self.node_data])
        pass
...
class MyDataset(Dataset):
    ...
    def __init__(self, data_root_path, image_size=256):
        super().__init__()
        self.data_root_path = data_root_path
        self.image_size = image_size

        self.sketch_flow_txt = self.get_all_txt()

        self.data_set = [SketchFlowGraph(one_txt, self.image_size) for one_txt in tqdm(self.sketch_flow_txt)]
        pass

    def get_all_txt(self):
        all_video = os.listdir(self.data_root_path)
        all_txt_list = []
        for one_video in all_video:
            all_txt = glob.glob(os.path.join(
                self.data_root_path, "{}/25_40_25/{}/9/track_line".format(one_video, one_video), "*.txt"))
            all_txt = sorted(all_txt, key=lambda a: int(os.path.basename(os.path.splitext(a)[0])))
            all_txt_list.extend(all_txt)
            pass
        return all_txt_list
...
class Runner(object):
    ...
    def __init__(self, batch_size=64, train_print_freq=100, test_print_freq=50, is_sgd=True,
        ...
        data_train_path = '../data/sketch/ped2/training/sketch_flow'
        data_test_path = '../data/sketch/ped2/testing/sketch_flow'
        self.train_dataset = MyDataset(data_root_path=data_train_path)
        self.test_dataset = MyDataset(data_root_path=data_test_path)
        ...
        if is_sgd:
            self.lr_s = [[0, 0.0001], [100, 0.00001]]
